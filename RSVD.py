...
def main():
    ...
    path = "./Tile_Zoo/h_defect_5x5"
    A = np.loadtxt(path,delimiter=',',dtype='complex_')
    k = 20
    U,S,V = rsvd(A,k)
    print("\n")
    print(U.shape)
    ...
    print(V.shape)
    print(S)
    A_trunc = np.multiply(S,V)
    A_trunc = np.einsum('ij,jk->ik',U,A_trunc)
    print(np.linalg.norm(A - A_trunc))
