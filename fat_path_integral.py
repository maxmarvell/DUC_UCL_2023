...
def path_integral(T:float, d:int, q:int):
    ...
    def transfer_matrix(t:float, X:float, Tile_Zoo:dict, a: np.ndarray,
        ...
        if not terminate:
            ...
            #return a[q**(2*(dim_v%d - 1))] if status == "Low Trim" else a[q**(2*(d-1))]
            return a[1]
    ...
    Tile_Zoo = dict()
    Tile_Zoo["Main h_direct"] = np.loadtxt(f"./Tile_Zoo/h_direct_{d}x{d}",delimiter=',',dtype='complex_')
    Tile_Zoo["Main h_defect"] = np.loadtxt(f"./Tile_Zoo/h_defect_{d}x{d}",delimiter=',',dtype='complex_')
    Tile_Zoo["Main v_direct"] = np.loadtxt(f"./Tile_Zoo/v_direct_{d}x{d}",delimiter=',',dtype='complex_')
    Tile_Zoo["Main v_defect"] = np.loadtxt(f"./Tile_Zoo/v_defect_{d}x{d}",delimiter=',',dtype='complex_')
    ...
    for t in range(1, int(2*T)):
        for x in range(-t, t):
            ...
            if (dim_v%d) != 0:
                Tile_Zoo["Low Trim h_direct"] = np.loadtxt(f"./Tile_Zoo/h_direct_{d}x{dim_v%d}",delimiter=',',dtype='complex_')
                Tile_Zoo["Low Trim h_defect"] = np.loadtxt(f"./Tile_Zoo/h_defect_{d}x{dim_v%d}",delimiter=',',dtype='complex_')
                Tile_Zoo["Low Trim v_direct"] = np.loadtxt(f"./Tile_Zoo/v_direct_{d}x{dim_v%d}",delimiter=',',dtype='complex_')
            ...
            if (dim_h%d) != 0:
                Tile_Zoo["Left Trim h_direct"] = np.loadtxt(f"./Tile_Zoo/h_direct_{dim_h%d}x{d}",delimiter=',',dtype='complex_')
            ...
            if (dim_v%d) != 0 and (dim_h%d) != 0:
                Tile_Zoo["Corner h_direct"] = np.loadtxt(f"./Tile_Zoo/h_direct_{dim_h%d}x{dim_v%d}",delimiter=',',dtype='complex_')
